...
function DetailItem({ data, detailPageId, setDetailPageId }) {
  ...
  const description =
    detailData[0].properties.Description.rich_text[0].plain_text;
  const imgSrc =
    detailData[0].cover.file?.url || detailData[0].cover.external.url;
  const disabled = () => {
  ...
  return (
    <section className="text-gray-600 body-font" style={{ width: "80vw" }}>
      <div
        className="flex lg:flex-row flex-col container 
